...
{def $current_user = fetch( 'user', 'current_user' )
     $mod_group = ezini('IDs', 'ModeratorGroupObjectID', 'xrowforum.ini')
     $bbcodes = ezini('BB-Codes','BBCodeList','xrowforum.ini')
     $close_and_sticky_access=fetch( 'user', 'has_access_to', hash( 'module',   'xrowforum',
                                                         'function', 'close',
     ...
     $parent=fetch( 'content', 'node', hash( 'node_id', $object.current.main_parent_node_id ) )
}

{if $related|count()|gt(0)}
    {foreach $related as $rel_object}
        {if $parent.path_string|begins_with($rel_object.main_node.path_string)}
...
<div class="edit">
    <div class="class-forum-topic">
        ...
        <form enctype="multipart/form-data" method="post" action={concat( "/content/edit/", $object.id, "/", $edit_version, "/", $edit_language|not|choose( concat( $edit_language, "/" ), '' ) )|ezurl}>
            ...
            <div class="block">
                <h3>{'Subject'|i18n('extension/xrowforum')}</h3>
            ...
            <div id="Topic_Content" class="block">
                <h3>{'Message'|i18n('extension/xrowforum')}</h3>
            ...
            <h3>{'Notify me about updates'|i18n('extension/xrowforum')}</h3>
            {attribute_edit_gui attribute=$object.data_map.notify_me}
            {if or(and($close_and_sticky_access, $moderator_here|eq(true)),and($policies.0.moduleName|eq('*'),$policies.0.functionName|eq('*')))}
                <h3>{'Sticky'|i18n('extension/xrowforum')}</h3>
                {attribute_edit_gui attribute=$object.data_map.sticky}
                <h3>{'Closed'|i18n('extension/xrowforum')}</h3>
                {attribute_edit_gui attribute=$object.data_map.closed}
            {/if}    
